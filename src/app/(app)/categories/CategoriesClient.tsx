...
export function CategoriesClient({ categories, rules }: { categories: Category[]; rules: MerchantRule[] }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="card">
        <h2 className="font-medium mb-3">Merchant rules</h2>
        ...
        <div className="overflow-y-auto max-h-96">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rules.map((r) => (
                <tr key={r.id}>
                  <td className="table-cell">{r.keyword}</td>
                  <td className="table-cell">{r.suggested_category_id ? catById.get(r.suggested_category_id)?.name : "—"}</td>
                  <td className="table-cell text-right">
                    <button
                      className="text-red-600 text-xs underline"
                      onClick={() => {
                        if (confirm("Delete this rule?")) start(async () => {
  ...
  return (
    <li className="flex items-center justify-between py-2">
      ...
      <div className="flex gap-2 text-xs">
        {editing ? (
          <>
            <button className="text-brand underline" onClick={() => start(async () => { await onSave(name); setEditing(false); })} disabled={pending}>Save</button>
            <button className="text-slate-600 underline" onClick={() => { setName(cat.name); setEditing(false); }}>Cancel</button>
          </>
        ) : (
          <>
            <button className="text-slate-600 underline" onClick={() => setEditing(true)}>Edit</button>
            <button className="text-red-600 underline" onClick={() => start(() => onDelete())} disabled={pending}>Delete</button>
          </>
        )}
      </div>
    </li>
